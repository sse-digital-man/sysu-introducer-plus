...
from ..info import ModuleInfo, ModuleDockerInfo
...
def _load_docker(
    ...
    raw_docker: Dict[str, Dict[str, Any]] = load_json(path)

    docker_config = {}
    # 读取关于 Docker 相关的配置信息
    for name, raw_module_docker in raw_docker.items():
        ...
        for kind, instance in module.items():
            ...
            docker_config[name][kind] = ModuleDockerInfo(
                instance["tag"], instance["envs"], format_ports, instance["isDaemon"]
            )
...
class ModuleLoader:
    ...
    def load(self):
        ...
        # 5. 设置 docker 配置信息
        for name, module_docker_config in _load_docker(DOCKER_CONFIG_PATH).items():
            module_cells[name].set_docker(module_docker_config)
