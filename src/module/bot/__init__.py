...
"""
    ...
    def talk(self, query: str) -> str:
        # 检索可参考的相似文档
        retrieve_res = self.retrieve_sim_k(query, 3)

        # 拼接回答，添加 few-shot 示范 以及 检索增强资料
        data_str = ""
        for _id, (_query, _document) in enumerate(retrieve_res.items()):
            data_str += self.data_prompt.format(i=_id + 1, query=_query, document=_document)

        final_query = self.demonstrate_prompt.format(query=query, data_str=data_str)

        return self._sub_module("caller").single_call(final_query, True)
